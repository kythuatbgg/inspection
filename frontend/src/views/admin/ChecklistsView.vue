<template>
  <div class="space-y-5 pb-24 md:pb-0 min-w-0">
    ...
    <div class="flex items-center justify-between gap-3">
      <div class="min-w-0">
        <h2 class="text-lg font-semibold text-slate-900 truncate">Quản lý Checklist</h2>
      ...
      <div class="hidden md:block bg-white rounded-[16px] border border-slate-200 shadow-sm overflow-x-auto">
        <table class="w-full">
          <thead class="bg-slate-50/80 border-b border-slate-200">
            <tr>
              <th class="px-5 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Tên checklist</th>
              <th class="px-5 py-4 text-center text-xs font-bold text-slate-500 uppercase tracking-wider">Hạng mục</th>
              <th class="px-5 py-4 text-center text-xs font-bold text-slate-500 uppercase tracking-wider">Điểm đạt</th>
              <th class="px-5 py-4 text-center text-xs font-bold text-slate-500 uppercase tracking-wider">Lỗi tối đa</th>
              <th class="px-5 py-4 text-center text-xs font-bold text-slate-500 uppercase tracking-wider">Trạng thái</th>
              <th class="px-5 py-4 text-right text-xs font-bold text-slate-500 uppercase tracking-wider">Thao tác</th>
            </tr>
          </thead>
          <tbody class="divide-y divide-gray-100">
            <tr v-for="c in checklists" :key="c.id" class="hover:bg-slate-50 transition-colors">
              ...
              <td class="px-5 py-4 text-center">
                <span v-if="c.has_active_batch" class="px-2.5 py-1 text-xs font-bold rounded-lg bg-amber-100 text-warning">Đang dùng</span>
                <span v-else class="px-2.5 py-1 text-xs font-bold rounded-lg bg-green-100 text-success">Rảnh</span>
              </td>
        ...
        <div v-if="checklists.length === 0" class="text-center py-10 text-slate-500">Chưa có checklist nào</div>
      </div>
      ...
      <div class="md:hidden space-y-3">
        <router-link v-for="c in checklists" :key="'m-'+c.id" :to="{ name: 'admin-checklist-detail', params: { id: c.id } }" class="block bg-white rounded-[18px] p-4 shadow-sm border border-slate-200 active:scale-[0.99] transition-transform">
          <div class="flex items-start justify-between gap-3 mb-3">
            ...
            <span v-if="c.has_active_batch" class="px-2 py-1 text-[10px] font-bold rounded-lg bg-amber-100 text-warning shrink-0">Đang dùng</span>
            <span v-else class="px-2 py-1 text-[10px] font-bold rounded-lg bg-green-100 text-success shrink-0">Rảnh</span>
          </div>
          <div class="grid grid-cols-3 gap-3 text-center">
            <div class="bg-slate-50/80 rounded-lg p-2.5">
              <p class="text-lg font-black text-slate-900">{{ c.items_count }}</p>
              <p class="text-[10px] text-slate-500 font-medium">Hạng mục</p>
            </div>
            <div class="bg-slate-50/80 rounded-lg p-2.5">
              <p class="text-lg font-black text-primary-600">{{ c.min_pass_score ?? '—' }}</p>
              <p class="text-[10px] text-slate-500 font-medium">Điểm đạt</p>
            </div>
            <div class="bg-slate-50/80 rounded-lg p-2.5">
              <p class="text-lg font-black text-danger">{{ c.max_critical_allowed ?? '—' }}</p>
              <p class="text-[10px] text-slate-500 font-medium">Lỗi tối đa</p>
            </div>
          </div>
          <div v-if="c.has_active_batch" class="mt-3 pt-3 border-t border-slate-200 flex justify-end">
            <button @click.prevent="cloneChecklist(c)" class="px-3 py-1.5 text-xs font-bold text-primary-600 bg-primary-50 rounded-lg">Clone bản mới</button>
          </div>
        </router-link>
        <div v-if="checklists.length === 0" class="text-center py-8 text-slate-500">Chưa có checklist nào</div>
      </div>
    </template>
    ...
    <div v-if="showCreateModal" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" @click.self="showCreateModal = false">
      <div class="bg-white rounded-lg p-6 w-full max-w-md shadow-2xl">
        <h3 class="text-lg font-bold text-slate-900 mb-4">Tạo Checklist mới</h3>
        <div class="space-y-4">
          <div>
            <label class="text-sm font-medium text-slate-700 mb-1 block">Tên checklist <span class="text-red-500">*</span></label>
            <input v-model="createForm.name" type="text" placeholder="VD: Checklist kiểm tra tủ cáp quang" class="w-full px-3 py-2.5 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500" />
          </div>
    ...
    <div v-if="showDeleteConfirm" class="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" @click.self="showDeleteConfirm = false">
      <div class="bg-white rounded-lg p-6 w-full max-w-md shadow-2xl">
        <div class="flex items-start gap-4 mb-4">
          ...
          <div>
            <h3 class="text-lg font-bold text-slate-900">Xóa checklist?</h3>
            <p class="text-sm text-slate-500 mt-1">{{ deleteTarget?.name }}</p>
          </div>
        </div>
        <p class="text-sm text-slate-600 mb-4">Xóa checklist sẽ xóa luôn tất cả hạng mục bên trong. Thao tác không thể hoàn tác.</p>
