    ...
    <div class="bg-white rounded-lg border border-slate-200 shadow-sm overflow-hidden">
      <div class="px-6 py-4 border-b border-slate-200 flex items-center justify-between">
        <div class="flex items-center gap-3">
          ...
          <h2 class="text-lg font-bold text-slate-900">{{ $t('settings.storageManagement') }}</h2>
        </div>
      ...
      <div v-else-if="stats" class="p-6">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4 mb-6">
          ...
          <div class="rounded-lg bg-warning/10 border border-amber-100 p-4">
            <p class="text-xs font-bold text-warning uppercase tracking-wide mb-1">{{ $t('settings.orphanFiles') }}</p>
          ...
          <div class="rounded-lg bg-sky-50 border border-sky-100 p-4">
            <p class="text-xs font-bold text-sky-600 uppercase tracking-wide mb-1">{{ $t('settings.usedUploadFiles') }}</p>
          ...
          <div class="rounded-lg bg-success/10 border border-green-100 p-4">
            <p class="text-xs font-bold text-success uppercase tracking-wide mb-1">{{ $t('settings.spatieMedia') }}</p>
          ...
          <div class="rounded-lg bg-primary-50 border border-blue-100 p-4">
            <p class="text-xs font-bold text-primary-600 uppercase tracking-wide mb-1">{{ $t('settings.totalStorage') }}</p>
        ...
        <div class="mb-6 rounded-lg bg-slate-50 border border-slate-200 p-4 text-sm text-slate-600 space-y-1">
          <p><span class="font-semibold text-slate-800">{{ $t('settings.totalUploadFiles') }}</span> {{ stats.upload_files_count }} file · {{ stats.upload_files_size_mb }} MB</p>
          <p>{{ $t('settings.orphanFilesDescription') }}</p>
        </div>

        <!-- Cleanup Controls -->
        <div class="rounded-lg bg-slate-50 border border-slate-200 p-5">
          <h3 class="text-sm font-bold text-slate-700 mb-3">{{ $t('settings.cleanupTitle') }}</h3>
          <p class="text-xs text-slate-500 mb-4">{{ $t('settings.cleanupDescription') }}</p>

          <div class="flex items-center gap-3">
            <select v-model="cleanupHours" class="text-sm border border-slate-300 rounded-lg px-3 py-2 bg-white">
              <option :value="0">{{ $t('settings.allTempFiles') }}</option>
              <option :value="1">{{ $t('settings.olderThan1h') }}</option>
              <option :value="6">{{ $t('settings.olderThan6h') }}</option>
              <option :value="24">{{ $t('settings.olderThan24h') }}</option>
              <option :value="72">{{ $t('settings.olderThan3d') }}</option>
              <option :value="168">{{ $t('settings.olderThan7d') }}</option>
            </select>
            ...
            >
              ...
              <template v-else>
                ...
                <span>{{ $t('settings.runCleanup') }}</span>
              </template>
            </button>
          </div>
...
const runCleanup = async () => {
  if (cleanupHours.value === 0) {
    const confirmed = window.confirm(t('settings.confirmDeleteAll'))
    if (!confirmed) return
  }
