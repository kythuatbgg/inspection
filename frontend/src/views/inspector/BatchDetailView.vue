  ...
  <div class="space-y-5">
    <template v-if="!loading && batch">
      ...
      <div class="rounded-lg bg-white border border-slate-200 p-5">
        ...
        <h2 class="text-lg font-bold text-slate-900">{{ batch.name }}</h2>
      ...
      <div class="rounded-lg bg-white border border-slate-200 p-5">
        <div class="flex items-center justify-between mb-2">
          <span class="text-sm font-semibold text-slate-700">Tiến độ tổng</span>
          <span class="text-lg font-bold" :class="progress.pct === 100 ? 'text-success' : 'text-primary-600'">
        ...
        <div class="w-full h-3 bg-slate-100 rounded-full overflow-hidden">
          <div
            ...
            :class="progress.pct === 100 ? 'bg-success/100' : 'bg-primary-500'"
            :style="{ width: progress.pct + '%' }"
          ></div>
        </div>
        <p class="text-xs text-slate-500 mt-1.5">{{ progress.pct }}% hoàn thành</p>
      </div>

      <!-- Cabinet List -->
      <div>
        <h3 class="text-base font-bold text-slate-900 mb-3">Danh sách tủ cáp ({{ planDetails.length }})</h3>

        <div v-if="planDetails.length === 0" class="text-center py-10">
          <Save class="w-14 h-14 mx-auto text-gray-300" />
          <p class="font-semibold text-slate-700 mt-3">Chưa có tủ nào</p>
        </div>
        ...
        <div v-else class="space-y-3">
          ...
          >
              ...
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2">
                  ...
                  <span 
                    ...
                    class="text-xs font-semibold px-2 py-0.5 rounded-full bg-success/10 text-success"
                  >✓ Đã kiểm tra</span>
                  <span 
                    v-else 
                    class="text-xs font-semibold px-2 py-0.5 rounded-full bg-warning/10 text-warning"
                  >Chưa kiểm tra</span>
                </div>
            ...
            <div v-if="plan.inspection" class="mt-3 pt-3 border-t border-slate-200 flex items-center gap-2 flex-wrap">
              <span
                class="text-xs font-bold px-2.5 py-1 rounded-full shrink-0"
                :class="plan.inspection.final_result === 'PASS' ? 'bg-green-100 text-success' : 'bg-red-100 text-danger'"
              >
              ...
              <span v-if="plan.inspection.critical_errors_count > 0" class="text-xs font-bold px-2 py-1 rounded-full bg-danger/10 text-danger flex items-center gap-1 shrink-0">
