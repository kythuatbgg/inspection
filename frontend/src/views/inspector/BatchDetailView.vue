<template>
  <div class="space-y-5">
    <template v-if="!loading && batch">
      <!-- Back + Batch Info -->
      <div class="rounded-2xl bg-dark-surface border border-gray-700/50 p-5">
        <button @click="goBack" class="flex items-center gap-1 text-sm text-primary-400 font-medium mb-3 -ml-1 active:opacity-70">
          <ChevronLeft class="w-4 h-4" />
          Quay lại
        </button>
        <h2 class="text-lg font-bold text-gray-100">{{ batch.name }}</h2>
      ...
      <div class="rounded-2xl bg-dark-surface border border-gray-700/50 p-5">
        <div class="flex items-center justify-between mb-2">
          <span class="text-sm font-semibold text-gray-300">Tiến độ tổng</span>
          <span class="text-lg font-bold" :class="progress.pct === 100 ? 'text-green-600' : 'text-primary-400'">
            {{ progress.done }}/{{ progress.total }}
          </span>
        </div>
        <div class="w-full h-3 bg-dark-elevated rounded-full overflow-hidden">
          <div
            class="h-full rounded-full transition-all duration-500"
            :class="progress.pct === 100 ? 'bg-green-500/100' : 'bg-primary-500/100'"
      ...
      <!-- Cabinet List -->
      <div>
        <h3 class="text-base font-bold text-gray-100 mb-3">Danh sách tủ cáp ({{ planDetails.length }})</h3>

        <div v-if="planDetails.length === 0" class="text-center py-10">
          <Save class="w-14 h-14 mx-auto text-gray-300" />
          <p class="font-semibold text-gray-300 mt-3">Chưa có tủ nào</p>
        </div>
        ...
        <div v-else class="space-y-3">
          ...
          >
            <div class="flex items-center justify-between">
              <div class="flex-1 min-w-0">
                <div class="flex items-center gap-2">
                  <h4 class="font-bold text-gray-100">{{ plan.cabinet_code }}</h4>
                  <span 
                    v-if="plan.status === 'done'" 
                    class="text-xs font-semibold px-2 py-0.5 rounded-full bg-green-500/10 text-green-400"
                  >✓ Đã kiểm tra</span>
                  <span 
                    v-else 
                    class="text-xs font-semibold px-2 py-0.5 rounded-full bg-amber-500/10 text-amber-400"
                  >Chưa kiểm tra</span>
                </div>
            ...
            <div v-if="plan.inspection" class="mt-3 pt-3 border-t border-gray-700/30 flex items-center gap-2 flex-wrap">
              <span
                class="text-xs font-bold px-2.5 py-1 rounded-full shrink-0"
                :class="plan.inspection.final_result === 'PASS' ? 'bg-green-500/15 text-green-400' : 'bg-red-500/15 text-red-400'"
              >
                {{ plan.inspection.final_result === 'PASS' ? 'ĐẠT' : 'KHÔNG ĐẠT' }}
              </span>
              <span class="text-xs font-bold px-2.5 py-1 rounded-full bg-primary-500/10 text-primary-700 shrink-0">Điểm gốc: {{ plan.inspection.total_score }}</span>
              <span v-if="plan.inspection.critical_errors_count > 0" class="text-xs font-bold px-2 py-1 rounded-full bg-red-500/10 text-red-400 flex items-center gap-1 shrink-0">
