<template>
  <div class="relative w-full" :class="containerClass" ref="containerRef">
    ...
    >
      <span :class="selectedLabel ? 'text-slate-900 font-medium' : 'text-slate-400'" class="truncate">
        {{ selectedLabel || placeholder }}
      </span>
      <ChevronDown class="w-5 h-5 text-slate-400 shrink-0 ml-2 transition-transform duration-200" />
    </button>
    ...
    <Teleport to="body">
      <Transition name="dropdown">
        <div
          ...
          class="fixed z-[100] bg-white rounded-lg border border-slate-200 shadow-sm shadow-gray-200/50 overflow-hidden"
          :style="dropdownStyle"
          ref="dropdownRef"
        >
          ...
          <div class="bg-white rounded-t-[28px] shadow-2xl max-h-[70vh] flex flex-col overflow-hidden safe-bottom">
            ...
            <div class="overflow-y-auto overscroll-contain py-2">
              <button
                ...
                class="w-full flex items-center justify-between px-5 min-h-[56px] transition-colors active:bg-slate-50"
                :class="String(opt.value) === String(modelValue) ? 'bg-primary-50/60' : ''"
              >
                <span class="text-[15px] font-medium" :class="String(opt.value) === String(modelValue) ? 'text-primary-700' : 'text-slate-900'">
                  {{ opt.label }}
                </span>
