  ...
  <div v-if="visible" class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative bg-white rounded-lg shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
      ...
      <div class="flex items-center justify-between p-4 border-b border-slate-200">
        <h3 class="text-lg font-semibold text-slate-900">{{ isEdit ? $t('cabinet.editCabinet') : $t('cabinet.addCabinet') }}</h3>
      ...
      <form @submit.prevent="handleSubmit" class="p-4 space-y-4">
        <!-- Cabinet Code -->
        <div>
          <label class="block text-sm font-medium text-slate-700 mb-1">{{ $t('cabinet.cabinetCode') }} *</label>
          <input
        ...
        <div class="grid grid-cols-2 gap-4">
          <div>
            <label class="block text-sm font-medium text-slate-700 mb-1">{{ $t('cabinet.latitude') }} *</label>
            <input
              v-model="form.lat"
              type="text"
              class="input-field min-h-[56px]"
          ...
          <div>
            <label class="block text-sm font-medium text-slate-700 mb-1">{{ $t('cabinet.longitude') }} *</label>
            <input
              v-model="form.lng"
              type="text"
              class="input-field min-h-[56px]"
        ...
        <!-- Note -->
        <div>
          <label class="block text-sm font-medium text-slate-700 mb-1">{{ $t('common.note') }}</label>
          <textarea
            v-model="form.note"
            rows="3"
            class="input-field min-h-[56px]"
        ...
        <div class="flex gap-3 pt-4">
          <button type="button" @click="close" class="btn-secondary flex-1 min-h-[56px]">{{ $t('common.cancel') }}</button>
          <button type="submit" :disabled="loading" class="btn-primary flex-1 min-h-[56px] flex items-center justify-center gap-2">
            <Loader2 v-if="loading" class="animate-spin w-5 h-5" />
            <span v-else>{{ isEdit ? $t('common.update') : $t('common.addNew') }}</span>
          </button>
        </div>
      </form>
    </div>
