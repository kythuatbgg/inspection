  ...
  <div v-if="visible" class="fixed inset-0 z-50 flex items-center justify-center p-4">
    ...
    <div class="relative bg-white rounded-xl shadow-2xl w-full max-w-lg mx-4 max-h-[90vh] overflow-y-auto">
      ...
      <div class="flex items-center justify-between p-4 border-b border-gray-200">
        <h3 class="text-lg font-semibold text-gray-900">{{ isEdit ? 'Sửa tủ cáp' : 'Thêm tủ cáp mới' }}</h3>
      ...
      <form @submit.prevent="handleSubmit" class="p-4 space-y-4">
        <!-- Cabinet Code -->
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">Mã tủ cáp *</label>
          <input
