      ...
      <div v-for="(group, category) in groupedDetails" :key="category" class="mb-4 last:mb-0">
        ...
        <button @click="toggleCategory(category)" type="button" class="w-full bg-gray-50 px-4 py-3 rounded-xl mb-3 border border-gray-100 flex items-center justify-between hover:bg-gray-100 active:scale-[0.99] transition-all">
          <div class="flex items-center gap-2">
            ...
            <span class="text-xs font-medium text-gray-400">{{ getCategoryFailed(group) }} lỗi / {{ group.length }}</span>
          </div>
        ...
        <div v-show="!collapsedCategories[category]" class="space-y-3">
          <div v-for="detail in group" :key="detail.id" class="p-4 bg-gray-50 rounded-[14px] border border-gray-100 flex gap-4">
